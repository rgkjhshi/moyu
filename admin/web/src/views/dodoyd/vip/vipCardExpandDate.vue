  ...
  <div class="app-container">
    <el-card header="会员卡延期">
      <el-form ref="form" label-width="100px">
        ...
        <el-form-item label="延期天数" prop="mobile">
          <el-col :span="4">
            <el-input v-model="expandDays" placeholder="请输入要延期的天数" />
          </el-col>
        </el-form-item>
        <el-form-item>
          <el-col :span="4">
...
<script>
import { getVipCardInfo, vipCardExpandDate } from '@/api/dodoyd'
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      // 校验代扣数额
      if (!/^[1-9]\d*$/.test(this.expandDays)) {
        this.$message({ showClose: true, message: '请输入正确的数字', type: 'error' })
        return
      }
      vipCardExpandDate({
        cardNo: this.cardNo,
        expandDays: this.expandDays
      }).then(response => {
        this.$message({
          showClose: true,
